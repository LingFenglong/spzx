...
<template>
  <div class="login">
    <el-form class="form" :model="model" :rules="rules" ref="loginForm" @keyup.enter="submit">
  ...
  setup() {
    ...
    const state = reactive({
      ...
      refreshCaptcha: async () => {
          ...
          state.captchaSrc = `data:image/png;base64,${data.codeValue}`
      },
      btnText: computed(() =>
        ...
        state.loginForm.validate(async valid => {
          if (valid) {
            ...
            const { code, data, message } = await Login({
              ...
              password: md5(state.model.password).toString(),
              captcha: state.model.captcha,
              codeKey: state.model.codeKey
            })
            if (+code === 200) {
              ctx.$message.success({
