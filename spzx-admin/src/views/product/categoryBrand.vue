<template>
  ...
  <el-table :data="list" style="width: 100%">
    ...
    <el-table-column label="操作" align="center" width="200" #default="scope">
      <el-button type="primary" @click="updateShow(scope.row)" size="small">
        修改
      </el-button>
      <el-button type="danger" @click="remove(scope.row.id)" size="small">
        删除
      </el-button>
    </el-table-column>
  </el-table>
...
import { FindCategoryListByParentId } from "@/api/category";
import { GetCategoryBrandPageList, SaveCategoryBrand, UpdateCategoryBrand, DeleteCategoryBrand } from "@/api/categoryBrand.js";
...
const updateShow = (row) => {
  ...
  categoryBrand.value.categoryId = null
  dialogVisible.value = true;
};
...
const props = {
  ...
  async lazyLoad(node, resolve) {   // 加载数据的方法
    ...
    const { data } = await FindCategoryListByParentId(node.value);
