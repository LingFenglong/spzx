<template>
  ...
  <el-table :data="list" style="width: 100%">
    ...
    <el-table-column label="操作" align="center" width="200" #default="scope">
      ...
      <el-button
        v-if="
          scope.row.auditStatus == 1 && scope.row.status == 1
        "
        type="warning"
        plain
        size="small"
...
import { FindCategoryListByParentId } from '@/api/category.js'
import {
  GetProductPageList,
  SaveProduct,
  UpdateProductById,
...
const getBrand = async () => {
  const categoryId = categoryIdList.value[2]

  if (categoryId !== undefined) {
    const { data } = await FindBrandByCategoryId(categoryId)
    categoryBrandList.value = data
  }
}
